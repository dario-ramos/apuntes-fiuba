...
\begin{equation}
\int_{\overrightarrow{\beta}} \overrightarrow{f} d\overrightarrow{\beta} = \int_{a}^{b} \overrightarrow{f}(\overrightarrow{\beta}(t)) \underbrace{\cdot}_{prod. escalar} \underbrace{ \beta '(t) dt }_{d\overrightarrow{\beta}}
\end{equation}
...
\begin{equation}
\int_{\overrightarrow{r}} f(\overrightarrow{r}) d\overrightarrow{r} = \int_{a}^{b} f(\overrightarrow{r(t)}) ||r'(t)|| dt
\end{equation}
...
\begin{align}
\overrightarrow{r(t)} &= (r \cos t, r \sin t) \Rightarrow \int_{\overrightarrow{r}} 1 d\overrightarrow{r} = \int_{0}^{2 \pi} ||r'(t)|| dt = \int_{0}^{2 \pi} ||(-r \sin t, r \cos t)|| dt \\
&= \int_{0}^{2 \pi} \sqrt{r^2 \sin^2 (t) + r^2 \cos^2 (t)} dt = r \int_{0}^{2 \pi} \underbrace{ \sqrt{\sin^2(t) + \cos^2(t)} }_{1} dt = r \int_{0}^{2 \pi} dt \\
&= r (2 \pi - 0) = 2 \pi r
\end{align}
...
\begin{align}
\int_{\overrightarrow{\beta}} \overrightarrow{f} d\overrightarrow{\beta} &= \int_{a}^{b} \overrightarrow{f}(\overrightarrow{\beta}(t)) \cdot \beta'(t) dt \\
&= \int_{a}^{b} \begin{bmatrix}P(\overrightarrow{\beta(t)}) & Q(\overrightarrow{\beta(t)}) & R(\overrightarrow{\beta(t)})\end{bmatrix} \cdot \begin{bmatrix}x'(t) \\ y'(t) \\ z'(t)\end{bmatrix} dt \\
&= \int_{a}^{b} P(\overrightarrow{\beta(t)}) \underbrace{x'(t) dt}_{dx} + Q(\overrightarrow{\beta(t)}) \underbrace{y'(t) dt}_{dy} + R(\overrightarrow{\beta(t)}) \underbrace{z'(t) dt}_{dz}
\end{align}

Realizando un potencial abuso de notación:

\begin{equation}
\int_{\overrightarrow{\beta}} \overrightarrow{f} d\overrightarrow{\beta} = \int_{\overrightarrow{\beta}} P dx + Q dy + R dz
\end{equation}
...
\begin{align}
\int_{\overrightarrow{\overrightarrow{\beta}}} \overrightarrow{F} \cdot d\overrightarrow{\beta} &= \int_{a}^b \overrightarrow{F}(\overrightarrow{\beta}(t)) \cdot \overrightarrow{\beta'(t)} dt = \int_{a}^b \left[ \overrightarrow{F}(\overrightarrow{\beta}(t)) \cdot \frac{ \overrightarrow{\beta'(t)} }{||\overrightarrow{\beta'(t)}||} \right] ||\overrightarrow{\beta'(t)}|| dt \\
&= \int_{a}^b \left[ \overrightarrow{F}(\overrightarrow{\beta}(t)) \cdot \overrightarrow{T}(t) \right] ||\overrightarrow{\beta}'(t)|| dt
\end{align}
...
\begin{equation}
d\Phi(P_0) = \nabla\Phi(P_0) \cdot H = \Phi_{x_1}(P_0) dx_1 + \ldots + I_{x_n}(P_0) dx_n 
\end{equation}

Por otro lado, considérese la integral de línea del gradiente de $\Phi$ sobre la curva $\overrightarrow{\sigma}$:

\begin{equation}
\int_{\overrightarrow{\sigma}} \overrightarrow{\nabla}\Phi \cdot d\overrightarrow{\sigma} = \int_a^b \underbrace{ \overrightarrow{\nabla}\Phi( \overrightarrow{\sigma}(t) ) \overrightarrow{\sigma}'(t) }_{d\Phi(\overrightarrow{\sigma}(t))} dt = \Phi(\overrightarrow{\sigma}(b)) - \Phi(\overrightarrow{\sigma}(b)) = \Phi(B) - \Phi(A)
\end{equation}
...
\begin{equation}
\int_{\overrightarrow{\sigma}} \overrightarrow{f} d\overrightarrow{\sigma} = \int_C y dx + x dy
\end{equation}
...
\begin{equation}
\int_C y dx + x dy = \Phi(B) - \Phi(A) = 1 \cdot 0 - (-1 \cdot 0) = 0
\end{equation}
...
Entonces, dada la integral de línea $\int_{\overrightarrow{\sigma}} P dx + Q dy$, resulta $\overrightarrow{f} = (P, Q)$. Para que $\overrightarrow{f}$ sea CG, necesariamente:
...
\begin{enumerate}[(A)]
\item Si $f(x,y) = 1 \Rightarrow \iint_R dx dy = $ área del rectángulo $R$. 
...
\begin{align}
\iint_R f(x,y) dx dy &= \int_a^b \left( \int_{\phi_1(x)}^{\phi_2(x)} f(x,y) dy \right) dx \\
area(R) &= \int_a^b \left( \int_{\phi_1(x)}^{\phi_2(x)} dy \right) dx
\end{align}
...
\begin{align}
\iint_R f(x,y) dx dy &= \int_c^d \left( \int_{\phi_1(y)}^{\phi_2(y)} f(x,y) dx \right) dy \\
area(R) &= \int_c^d \left( \int_{\phi_1(y)}^{\phi_2(y)} dx \right) dy
\end{align}
...
\begin{equation}
\int_{-5}^0 \left( \int_{-1-y^2}^{y^2} f(x,y) dx \right) dy
\end{equation}
...
\begin{align}
\int_{-5}^0 \left( f(x,y) dx \right) dy &= \iint_{R_1} (f(x,y) dy) dx + \iint_{R_2} (f(x,y) dy) dx + \iint_{R_3} (f(x,y) dy) dx \\
\iint_{R_1} (f(x,y) dy) dx &= \int_{-26}^{-1} \left( \int_{-5}^{-\sqrt{-1-x}} f(x,y) dy  \right) dx \\
\iint_{R_2} (f(x,y) dy) dx &= \int_{-1}^{0} \left( \int_{-5}^0 f(x,y) dy \right) dx \\
\iint_{R_3} (f(x,y) dy) dx &= \int_{0}^{25} \left( \int_{-5}^{-\sqrt{x}} f(x,y) dy \right) dx
\end{align}
...
Es posible generalizar los conceptos de la integral doble partiendo de un recinto $R = [a,b] \times [c,d] \times [e,f]$, que consiste en un paralelepípedo en $\Bbb R^3$. Definiendo la integral triple sobre $R$ como $\iiint_R f(x,y,z) dx dy dz$, puede probarse que todos los resultados de la integral doble siguen valiendo: Fubbini, linealidad, etc.
...
\begin{equation}
\iiint_R f(x,y,z) \mathop{dx} \mathop{dy} \mathop{dz} = \int_0^1 \left( \int_0^1 \left( \int_0^{x^2} xyz^2 dz \right)  \right) dy) dx
\end{equation}
